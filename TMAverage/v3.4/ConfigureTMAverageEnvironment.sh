...
# Last Modified: February 11th, 2026 - RS
##########################################################################
...
EOD
    ...
    read_write_permissions=$("$HOME/common/oracle/GrantNewPermissions.sh" "$tables" ALL "$username")
    if [ $? -ne 0 ]; then
    ...
    tables="$tmanalog_table_name,$telemetry_item_definitions_name,$telemetry_analog_conversions_name"
    select_permissions=$("$HOME/common/oracle/GrantNewPermissions.sh" "$tables" SELECT "$username")
    if [ $? -ne 0 ]; then
        echo "$select_permissions"
        echo "An error occurred while granting read permissions to tables $tables on $username. Exiting..."
        exit 1
    fi
    ...
    if [ $otfd_opt -ne 0 ]; then
        ...
        packages="$misc_schema.ONTHEFLYDECOM,$misc_schema.ONTHEFLYDECOMMISSIONSPECIFIC"
        otfd_execute=$("$HOME/common/oracle/GrantNewPermissions.sh" "$packages" SELECT "$username")
        if [ $? -ne 0 ]; then
            echo "$otfd_execute"
        ...
        otfd_tables=$("$HOME/common/oracle/GrantNewPermissions.sh" "$tables" SELECT "$username")
        if [ $? -ne 0 ]; then
            echo "$otfd_tables"
