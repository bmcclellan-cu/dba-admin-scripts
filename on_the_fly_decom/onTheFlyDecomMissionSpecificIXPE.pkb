...
supportedSIDs SIDsVARRAY := SIDsVARRAY(1);
...
BEGIN
    ...
    IF    (type_in = 0) THEN
        tableName := 'IXPE_L0.L0_Packets_SID' || systemId_in;
    ELSIF (type_in = 1) THEN
        tableName := 'IXPE_L1A.TManalog_SID'   || systemId_in;
    ELSIF (type_in = 2) THEN
        tableName := 'IXPE_L1.TMdiscrete_SID' || systemId_in;
    ELSIF (type_in = 3) THEN
        tableName := 'IXPE_MISC.TelemetryStorageLocation';
    ELSIF (type_in = 4) THEN
        tableName := 'IXPE_MISC.TMDecom';
    ELSIF (type_in = 5) THEN
        return 'IXPE_CT.TelemetryItemDefinition';
    ELSIF (type_in = 10) THEN
        tableName := 'IXPE_L0.TelemetrySourceFiles';
    ELSE
        RAISE invalidType;
    END IF;
    RETURN tableName;
...
BEGIN
    ...
    IF not systemId_in MEMBER OF supportedSIDs THEN
        ONTHEFLYDECOM.logOTFD('getDefinitionStartStopTimes: IXPE does not support SID ' || systemId_in || '.', 0);
        RETURN 0;
    END IF;
